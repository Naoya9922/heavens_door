(fn => {
  if (
    ...
    if (typeof Turbolinks !== "undefined" && Turbolinks) {
      if (Turbolinks.EVENTS && Turbolinks.EVENTS.LOAD) {
        // turbolinks-classic
  ...
  Array.from(document.getElementsByTagName("form")).forEach(form => {
    form.addEventListener("submit", () => {
      if (sessionStorage.heavensDoor) {
        Array.from(form.querySelectorAll("input,textarea,select")).forEach(
          el => {
            ...
            if (
              ...
              if (el.value) {
                sessionStorage.heavensDoor += `    fill_in '${target}', with: '${
                  el.value
                }'\n`;
              }
            } else if (el.type == "date") {
              sessionStorage.heavensDoor += `    fill_in '${target}', with: Date.parse('${
                el.value
              }')\n`;
            } else if (el.type == "select-one") {
              sessionStorage.heavensDoor += `    select '${
        ...
        sessionStorage.heavensDoor += `    click_button '${
